...
/* ------- Сайдбар: подготовка DOM + вычисление высоты списка ------- */
function ensureSidebarHeader(){
  ...
  if (!sb.querySelector('.sidebar-header')){
    ...
    title.textContent = 'Точки маршрута';

    var btn = document.createElement('button');
    btn.id = 'btnCloseSidebar';
...
function adjustListHeight(){
  ...
  var header = sb ? sb.querySelector('.sidebar-header') : null;
  if (!sb || !list) return;
...
function buildList(){
  ...
  pointFeatures.forEach(function(f){
    ...
    item.innerHTML =
      '<h4 class="title">'+escapeHtml(cleanText(p.name) || 'Без названия')+'</h4>'+
...
function applyVisibility(){
  ...
  var items = Array.prototype.slice.call(document.querySelectorAll('#list .list-item'));
...
// «Меню»: открыть/закрыть, пересчитать высоту списка
