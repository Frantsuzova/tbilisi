// Основано на твоём текущем коде. Правки:
// 1) закрытие сайдбара по #btnCloseSidebar (работает с твоим HTML);
// 2) размеры сайдбара не на весь экран, пересчёт высоты списка;
...
/* ===== текст/HTML ===== */
function toText(v){ if (v==null) return ''; if (typeof v==='string') return v; if (typeof v==='number'||typeof v==='boolean') return String(v);
  if (Array.isArray(v)) return v.map(toText).filter(Boolean).join(' ');
  if (typeof v==='object'){ var prefer=['__cdata','#cdata-section','#text','text','value','content','description'];
...
function cleanText(v){ var s=toText(v); s=s.replace(/\[object Object\]/gi,' ').replace(/\s{2,}/g,' ').trim(); return s; }
function stripHtmlToText(input){ var html=cleanText(input); if(!html) return '';
  ...
  t=t.replace(/\[object Object\]/gi,'').replace(/\s{2,}/g,' ').trim(); return t; }
function escapeHtml(s){ s=String(s==null?'':s); return s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;'); }
function makePopupHtml(name, description){ var nameText=escapeHtml(cleanText(name))||'Без названия'; var descText=stripHtmlToText(description);
...
function idFromHref(href){ if(!href) return null; var fn=href.split('?')[0].split('#')[0].split('/').pop()||"";
  var m=fn.match(/(?:^|[^\d])([1-9]\d{0,2})(?=\D|$)/); if(!m) return null; var n=parseInt(m[1],10); return (n>=1&&n<=ICONS.count)?n:null; }
function buildStyleHrefMap(kmlXml){
  var byId=Object.create(null);
  var styles=kmlXml.querySelectorAll('Style[id]');
  for(var i=0;i<styles.length;i++){ var st=styles[i], id=st.getAttribute('id'), href=qText(st,'IconStyle Icon href'); if(id&&href) byId['#'+id]=href; }
...
/* ===== цвет и SVG-пин ===== */
function extractHexFromHref(href){ if(!href) return null; var m=href.match(/(?:[?&#]color=)(?:0x)?([0-9a-fA-F]{6,8})/); if(m){ var hex=m[1].toLowerCase();
  return '#'+(hex.length===8?hex.slice(2):hex).replace(/^([0-9a-f]{6}).*$/,'$1'); } return null; }
function guessNamedColorFromHref(href){ if(!href) return null; var s=href.toLowerCase();
...
function getStyleColor(feature,styleHrefMap){
  var p=(feature&&feature.properties)||{}; var styleUrl=typeof p.styleUrl==='string'?p.styleUrl:null;
  var href=styleUrl?(styleHrefMap[styleUrl]||null):null; var hex=extractHexFromHref(href);
  if(hex) return hex; hex=guessNamedColorFromHref(href); if(hex) return hex; return null;
}
function svgPinIcon(hex){
  var fill=(hex||'#2b7bff').toLowerCase(), stroke='#08213a', w=26, h=40, ax=Math.round(w/2), ay=h;
...
/* ===== fitBounds padding c учётом панелей ===== */
...
/* ===== сайдбар: закрытие + высота списка ===== */
function bindCloseSidebar(){
...
function renderGeoJSON(geojson, styleHrefMap){
  ...
  try{
    ...
    if (b.isValid()) map.fitBounds(b, getFitPadding()); else map.setView([41.6938,44.8015],14);
...
function updateCounters(){
  var total=pointFeatures.length, cats={ stairs:0, porches:0, temples:0, other:0 };
...
function buildList(){
  ...
  pointFeatures.forEach(function(f){
    ...
    item.innerHTML='<h4 class="title">'+escapeHtml(cleanText(p.name)||'Без названия')+'</h4>'+
...
function isMatchProps(p, activeCat, qLower){
  var cat=detectCategory(p), name=cleanText(p.name).toLowerCase(), desc=cleanText(p.description).toLowerCase();
...
function applyVisibility(){
  ...
  pointFeatures.forEach(function(f){
    var p=f.properties||{}; var show=isMatchProps(p, activeCat, q); if (!show) return;
...
function selectCategory(cat){
  ...
  for (var i=0;i<chips.length;i++){
    var ch=chips[i]; ch.dataset.active=(ch.dataset.cat===cat?'true':'false');
  }
...
var chipBtns=document.querySelectorAll('.chip'); for (var cb=0; cb<chipBtns.length; cb++){ (function(btn){ btn.addEventListener('click', function(){ selectCategory(btn.dataset.cat); }); })(chipBtns[cb]); }
