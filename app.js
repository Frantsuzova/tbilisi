...
// ---------- УТИЛИТЫ ТЕКСТА ----------
const toText=v=>v==null?'':Array.isArray(v)?v.map(toText).join(' '):typeof v==='object'?Object.values(v).map(toText).join(' '):String(v);
...
function detectCat(p){
  const n=clean(p?.name).toLowerCase(), d=clean(p?.description).toLowerCase();
...
const inferAnchor = href => (/paddle|pin|pushpin|marker|kml\/paddle/i.test(href||'')) ? [ICON_SIZE[0]/2, ICON_SIZE[1]] : [ICON_SIZE[0]/2, ICON_SIZE[1]/2];
const iconRemote  = href => L.icon({iconUrl:href, iconSize:ICON_SIZE, iconAnchor:inferAnchor(href), popupAnchor:[0,-18]});
const iconSvgPin  = hex => {
  ...
  return L.divIcon({className:'pin-svg', html, iconSize:[w,h], iconAnchor:[ax,ay], popupAnchor:[0,-30]});
};

// «Буквенные/служебные» — пропускаем из локального маппинга
const isLetterish = href => /(waypoint|letters?[-_/]|dir[-_]|direction|route|track[-_]|letter[-_a-z]*\.png)/i.test(href||'');

// Список реально существующих локальных /tbilisi/icon-N.png (без 404)
async function detectLocalIcons(maxTry=80){
  const base = location.pathname.replace(/\/[^/]*$/,''); // /tbilisi
  const urls=[];
  for(let n=1;n<=maxTry;n++){
    const candidates=[`${base}/icon-${n}.png`, `/icon-${n}.png`, `./icon-${n}.png`, `icon-${n}.png`];
    let okUrl=null;
    // eslint-disable-next-line no-await-in-loop
    for(const u of candidates){ if(await probe(u)){ okUrl=u; break; } }
    if(okUrl) urls.push(okUrl); else break; // как только очередной не нашли — выходим
  }
  return urls;
}
...
const tiles=L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png',{subdomains:'abcd',maxZoom:20,attribution:'&copy; OpenStreetMap &copy; CARTO'}).addTo(map);
...
// ---------- СОСТОЯНИЕ ----------
let styleMaps=null, localIconUrls=[], styleUrlToLocal=Object.create(null);
let shapesLayer=null, markerGroup=L.layerGroup().addTo(map), markersById=new Map(), pointFeatures=[], boundsAll=null;
let lastSummaryBase='';

// Стили линий/полигонов
...
// Подготовка соответствия styleUrl → локальный icon-N (только для «небуквенных»)
async function prepareLocalIconMapping(points){
  styleUrlToLocal=Object.create(null);
  if(!localIconUrls.length) return;

  // уникальные styleUrl по порядку объявления в KML, только с нормальными иконками
  const used = [...new Set(points.map(f=>normStyleUrl(f.properties?.styleUrl)).filter(Boolean))];
  used.sort((a,b)=>(styleMaps.orderIdx[a]||9999)-(styleMaps.orderIdx[b]||9999));

  const eligible = used.filter(su=>{
    const href = styleMaps.styles[su]?.iconHref || '';
    return href && !isLetterish(href);
  });

  // ограничиваем числом реально существующих локальных PNG
  const take = Math.min(eligible.length, localIconUrls.length);
  for(let i=0;i<take;i++){
    styleUrlToLocal[eligible[i]] = localIconUrls[i];
  }
}
...
async function renderGeoJSON(geojson){
  ...
  await prepareLocalIconMapping(pointFeatures);
  ...
  const layer=L.geoJSON(pointFeatures,{
    pointToLayer:(feature,latlng)=>{
      ...
      // Базовый — наш SVG-пин нужного цвета (гарантированно есть)
      const m=L.marker(latlng,{icon:iconSvgPin(hex)});

      // 1) если есть локальный PNG для этого стиля — используем его
      const localUrl = styleUrlToLocal[su];
      if(localUrl){
        m.setIcon(L.icon({iconUrl:localUrl, iconSize:ICON_SIZE, iconAnchor:[ICON_SIZE[0]/2,ICON_SIZE[1]/2], popupAnchor:[0,-16]}));
      } else if (href){
        // 2) иначе пробуем оригинальный href из KML
        probe(href).then(ok=>{ if(ok) m.setIcon(iconRemote(href)); });
      }
...
(async()=>{
  try{
    // 1) локальные иконки — определяем, сколько реально есть (без 404)
    localIconUrls = await detectLocalIcons(); // например, ["/tbilisi/icon-1.png", "/tbilisi/icon-2.png", …]

    // 2) KML → стили → geojson → рендер
